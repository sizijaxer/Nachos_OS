...
Thread::Thread(char* threadName, int threadID)
{
	ID = threadID;
    name = threadName;
    priority = kernel->get_input_priority(ID);
    //cout<<name<<"pri: "<<priority<<"\n";
    burstTime = 0;
    startBurst = 0;
    ExecTime = 0;
    waitingTime = 0;
...
Thread::Yield () // Running->Ready
{
    ...
    if (nextThread != NULL) {
        ...
        /*if (nextThread->getPriority()>=100) {///L1
        ...
        else if (nextThread->getPriority()<=99 && nextThread->getPriority()>=50){///L2
            ...
            /*if(this->getPriority()<50){// this in or L3
                kernel->scheduler->ReadyToRun(this);
                this->setReadyTime();
                kernel->scheduler->Run(nextThread, FALSE);
            }
        ...
        else if (nextThread->getPriority() < 50){///L3
...
void Thread::setPriority(int new_priority){
    ...
    if(new_priority > 149)priority = 149;
    else priority = new_priority;
    DEBUG('z',"[C] Tick ["<<kernel->stats->totalTicks<<"]: Thread ["<<ID<<"] changes its priority from ["<<old_priority<<"] to ["<<priority<<"]");
}
...
void Thread::setReadyTime(){
    ReadyTime = kernel->stats->totalTicks;
}

int Thread::getReadyTime(){
    return ReadyTime;
}
void Thread::setExecTime(double t){
    ExecTime = t;
}
int Thread::getExecTime(){
    return ExecTime;
}

void Thread::setBurstTime(double sb, double b){
    ExecTime = kernel->stats->totalTicks - sb;
    burstTime = ((kernel->stats->totalTicks - sb) + b)/2;
    // t(i) = 0.5*T + 0.5*t(i-1)
    DEBUG('z',"[D] Tick ["<<kernel->stats->totalTicks<<"]: Thread ["<<ID<<"] update approximate burst time, from: ["<<kernel->stats->totalTicks - sb<<"], add ["<<b<<"], to ["<<burstTime<<"]");
}
